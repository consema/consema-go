...
// product version (the productVersion declaration in
// cmd/consema/version.go — the declaration is the anchor, line numbers
// may drift; the literal intentionally does not appear in this comment,
// so the fixture-count gate counts only fixtures — wave-5 P2), matching
// what the real CLI emits. These fixture literals ride the release train:
// the check-version-consistency CI gate asserts their exact count at the
// current version (wave-4 R36, 2026-08-15 — the gate previously never
// touched this file, so a train bump silently left the fixtures stale;
// this file is package protocol and cannot reference the package-main
// version constant, so the gate count is the sync point).
