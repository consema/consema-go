...
// executes all 22 published java-properties-v1 cases with zero skips and
...
func TestJavaPropertiesV1SuitePins(t *testing.T) {
	...
	if len(data.Cases) != 22 {
		t.Fatalf("case count %d != 22", len(data.Cases))
	}
	report := runJavaPropertiesV1(runner, data)
	if len(report.Passed) != 22 {
		for _, failure := range report.Failed {
			t.Errorf("case %s: %s", failure.ID, failure.Message)
		}
		t.Fatalf("java-properties-v1: %d passed, %d skipped, %d failed; want 22 passed",
