...
// `optionxform` semantics (crates/consema-ini python_case.rs). The tables
...
// from changing an already selected INI profile.

// lowerRange is one ordered Unicode 16.0 full-lowercase range: every code
// point congruent to start modulo step within [start, end] maps by delta.
type lowerRange struct {
	start, end, step uint32
	delta            int32
}

// lowerSingle is one isolated full-lowercase mapping.
type lowerSingle struct {
	upper, lower uint32
}

var lowerRanges = []lowerRange{
...
func optionxform(value string) string {
	...
	for _, character := range value {
		...
		if code == 0x000130 {
			output = append(output, 'i', 0xCC, 0x87) // U+0305 combining dot above in UTF-8
			continue
		}
		if mapped, ok := simpleLowercase(code); ok {
			output = appendRuneUTF8(output, rune(mapped))
			continue
		}
		output = appendRuneUTF8(output, character)
	}
...
func simpleLowercase(code uint32) (uint32, bool) {
	...
	if index > 0 {
		candidate := lowerRanges[index-1]
		if code <= candidate.end && (code-candidate.start)%candidate.step == 0 {
			return uint32(int64(code) + int64(candidate.delta)), true
		}
	}
	index = sort.Search(len(lowerSingles), func(i int) bool {
		return lowerSingles[i].upper >= code
	})
	if index < len(lowerSingles) && lowerSingles[index].upper == code {
		return lowerSingles[index].lower, true
	}
	return 0, false
}

// appendRuneUTF8 appends one scalar in UTF-8 without importing unicode/utf8.
func appendRuneUTF8(output []byte, character rune) []byte {
	switch {
	case character < 0x80:
		return append(output, byte(character))
	case character < 0x800:
		return append(output,
			byte(0xC0|character>>6), byte(0x80|character&0x3F))
	case character < 0x10000:
		return append(output,
			byte(0xE0|character>>12), byte(0x80|(character>>6)&0x3F),
			byte(0x80|character&0x3F))
	default:
		return append(output,
			byte(0xF0|character>>18), byte(0x80|(character>>12)&0x3F),
			byte(0x80|(character>>6)&0x3F), byte(0x80|character&0x3F))
	}
}
