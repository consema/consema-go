...
// This file implements the Properties-local edit records: the change set
// and the transferable dry-run plan (consema-document change.rs,
// edit_plan.rs; RFC 0016 §5.3). go/document does not yet carry these
// records; they live here until the document milestone that absorbs them.
...
// NodeMapping is one old-to-new node mapping fact (consema-document
// ChangeSet node mappings).
...
func newEditPlan(sourceID string, profile document.ProfileId,
	...
	for index, operation := range operations {
		...
		expected := operation.Operation.ID() + "@" + uint32String(operation.Operation.Version())
