...
- 同步范围：`feature_complete_judgment.open_items[].evidence`（C-2 条）残留行号引用 `docs/fuzz-evidence-0.13.0.md`（行号已删除）→ 母仓现行节锚 `docs/fuzz-evidence-0.13.0.md §7（完成路径）`；W3-12/F7 重同步随母仓注记字符串行号删除。
