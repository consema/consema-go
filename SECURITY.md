...
`toml.1.0@1` 只对完整合法文档形成 snapshot，非法输入返回 `FormationFailure`。本仓发布流程（release.yml）没有 toml-test 门禁——tag 即发布，tag 路径重跑 Go 门禁（gofmt/vet/build/test/race，含 conformance 数据）与四个跨语言差分 harness（byte parity / normalized / protocol exchange / shared conformance；G026，对抗审计 2026-08-14：旧文「发布验证仅重跑 Go 门禁」漏掉 release.yml 的 differential job），Go proxy 自动拾取待 `consema.dev` 域名与模块发布就绪；TOML 一致性由 conformance 套件（toml-v1 向量 18 cases、fixtures/toml 真实夹具含 cargo-manifest）与 go/toml 包测试覆盖。semantic edit 不会舍入 NaN payload、亚纳秒时间或非整分钟 offset 来伪造成功。
