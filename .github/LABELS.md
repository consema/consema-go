...
`unlabeled`, so adding a label flips it green without a rebase. Wave-5 P2
disclosure: the check is not among the branch-protection required status
checks (the only required context is `check (all gates green)`), so a
missing `kind:` label shows a red check but does not block the merge
button by itself.
...
| `area: core`    | `d93f0b` | Implementation / authoritative content (repo source tree: `go/**`; root meta files `LICENSE`/`.gitignore`/`.gitattributes`) |
| `area: ci`      | `bfd4f2` | CI / tooling: `.github/**`, `scripts/**`                           |
| `area: docs`    | `c2e0c6` | Documentation: `*.md`                                                   |
| `needs-info`    | `d4c5f9` | Issues awaiting information: the stale workflow (`only-labels: needs-info`) target |
