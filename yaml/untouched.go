...
func (e *UntouchedByteProofError) Error() string {
	...
	case ProofErrorEncodingMismatch:
		return "yaml: untouched-byte proof encoding mismatch"
	case ProofErrorInvalidReplacement:
		return fmt.Sprintf("yaml: untouched-byte proof invalid replacement at %d", e.Index)
	case ProofErrorReplacementOrder:
		return fmt.Sprintf("yaml: untouched-byte proof replacement order at %d", e.Index)
	case ProofErrorDuplicateInsertion:
		return fmt.Sprintf("yaml: untouched-byte proof duplicate insertion at %d", e.Index)
	case ProofErrorOriginalMismatch:
		return fmt.Sprintf("yaml: untouched-byte proof original mismatch at %d", e.Index)
	case ProofErrorTargetMismatch:
		return "yaml: untouched-byte proof target mismatch"
	case ProofErrorCoordinateOverflow:
		return "yaml: untouched-byte proof coordinate overflow"
	case ProofErrorInvalidRegion:
		return fmt.Sprintf("yaml: untouched-byte proof invalid region at %d", e.Index)
	case ProofErrorDigestMismatch:
		return "yaml: untouched-byte proof digest mismatch"
	case ProofErrorProofMismatch:
		return "yaml: untouched-byte proof region mismatch"
	}
	return "yaml: untouched-byte proof error"
}
